...
public class RegressionTests extends BaseClass {
    ...
    public List<String> menuBebe = Arrays.asList(
            "Camisas","Camisetas", "Sudaderas", "Jackets", "Vestidos", "Denim", "Pantalones", "Shorts",
            "Trajes de Baño", "Medias", "Zapatos", "Accesorios", "Pijamas", "OnePiece & Sets");
    ...
    public void validar_elementos_submenu() {
        ...
        for(String opcion: opciones) assess.assertTrue(menuBebe.contains(opcion));
        assess.assertAll();
    }
    ...
    public void validar_ordenar_busqueda_precio_ascendente() {
        ...
        SoftAssert assess = new SoftAssert();
        for(int i=1; i < precios.size(); i++) assess.assertTrue(precios.get(i-1)<= precios.get(i));
        assess.assertAll();
    }
    ...
    public void validar_filtros_busqueda() {
        ...
        SoftAssert assess = new SoftAssert();
        for(String marca: marcas) assess.assertEquals(marca, "Athleta", "Hay marcas distintas a 'Athleta " +
                "en la busqueda. Revise los filtros'");
        assess.assertAll();
    }
}
